...
func Worker(mapf func(string, string) []KeyValue, reducef func(string, []string) string) {
	// start worker
	for {
		// worker get a task from master
		task := getTask()

		// Moore to distribute task based on state
...
func reducer(task *Task, reducef func(string, []string) string) {
	// read all KeyValue in intermediate based on filepath
...
func mapper(task *Task, mapf func(string, string) []KeyValue) {
	//read content based on fliename
	content, err := ioutil.ReadFile(task.Input)
	if err != nil {
		log.Fatal("Failed to read file: "+task.Input, err)
	}
	//use mapf，Cache results
	intermediates := mapf(task.Input, string(content))

	//cached result wirted local and silced into n part
	//based on "ihash(key)"
	/*example: current k&v{
		word(string)	:apple
		count(string)	:1
	...
	*/
	...
	//send intermediate address to master
	task.Intermediates = mapOutput
	TaskCompleted(task)
}

func getTask() Task {
	// worker gets task from master
	args := ExampleArgs{}
	reply := Task{}
...
func TaskCompleted(task *Task) {
	//send RPC:"Master.TaskCompleted" to master 
	reply := ExampleReply{}
